...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		if(movedPiece instanceof Pawn) {
			boolean whitePromotion = movedPiece.getColor() == Color.WHITE && target.getRow() == 0;
			boolean blackPromotion = movedPiece.getColor() == Color.BLACK && target.getRow() == 7;
	...
	private Piece makeMove(Position source, Position target) {
		...
		// special move castling rook kingside rook
		...
		// special move castling rook queenside rook
		if (p instanceof King && target.getColumn() == (source.getColumn() - 2)) {
	...
	private void validateSourcePosition(Position position) {
		...
		if (!board.piece(position).isThereAnyPossibleMove()) {
			throw new ChessException("There is no possible moviments for the chosen position");
		}
	}
